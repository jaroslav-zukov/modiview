...
app.layout = html.Div(
    [
        ...
        dcc.Dropdown(id="mod_dropdown", multi=True),
...
@app.callback(
    [Output("read_number", "value"), Output("zoom-range", "data")],
    [
        Input("uploaded-file-path", "data"),
...
def update_output(value, file_path):
    ...
    bam_handler = BAMFileHandler(file_path)
    return "You have selected read number {} with read_id {}".format(
        value, bam_handler.get_read_id(value)
    )
        ...
        df = pd.DataFrame(
            {
                ...
                "y": modifications.generate_plot_data(
                    modification, read, nucleotides_shown
                )[start:end],
            }
        )
        fig.add_trace(go.Scatter(x=df["x"], y=df["y"], mode="lines", name=modification))
...
@app.callback(Output("previous-file-path", "data"), Input("uploaded-file-path", "data"))
